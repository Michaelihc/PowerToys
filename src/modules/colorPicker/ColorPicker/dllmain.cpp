...
private:
    ...
    void launch_process(bool exit_after_close = false)
    {
        ...
        Logger::info(L"Starting ColorPicker process. exit_after_close={}", exit_after_close);
        ...
        if (ShellExecuteExW(&sei))
        {
            ...
            Logger::info("Successfully started the Color Picker process. pid={}", GetProcessId(m_hProcess));
        }
        else
        {
...
public:
    ...
    virtual bool on_hotkey(size_t /*hotkeyId*/) override
    {
        if (m_enabled)
        {
            ...
            if (!is_process_running())
            {
                launch_process(!PTSettingsHelper::should_fast_launch(get_key()));
            }

...
namespace PTSettingsHelper
{
    ...
    namespace
    {
        ...
        {
            json::JsonObject low_memory_settings;
            if (json::has(general_settings, low_memory_modules_json_field_name, json::JsonValueType::Object))
            {
                low_memory_settings = general_settings.GetNamedObject(low_memory_modules_json_field_name);
            }
            else
            {
                low_memory_settings = create_default_low_memory_module_settings();
            }
        ...
        {
            {
                ...
                {
                    return low_memory_settings_cache;
                }
            }

            refresh_low_memory_settings_cache();

            std::lock_guard lock{ low_memory_settings_mutex };
            if (!low_memory_settings_cache_initialized)
            {
                return create_default_low_memory_module_settings();
            }
    ...
    bool low_memory_mode_enabled()
    {
        try
        {
            ...
            return is_any_low_memory_module_enabled(low_memory_modules);
        }
        catch (...)
        {
            return false;
